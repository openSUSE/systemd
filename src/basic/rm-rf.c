...
static int unlinkat_harder(int dfd, const char *filename, int unlink_flags, RemoveFlags remove_flags) {
        mode_t old_mode;
        int r;
...
static int fstatat_harder(int dfd,
                const char *filename,
                struct stat *ret,
                int fstatat_flags,
...
int rm_rf_children(
        ...
        struct dirent *de;
        size_t n_todo = 0, n_todo_allocated = 0;
                ...
                FOREACH_DIRENT_ALL(de, d, return -errno) {
                        ...
                        if (r == -EISDIR) {
                                /* Push the current working state onto the todo list */
                                 ...
                                 if (!GREEDY_REALLOC0(todos, n_todo_allocated, n_todo + 2))
                                         return log_oom();
