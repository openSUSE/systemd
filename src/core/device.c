...
static int device_set_sysfs(Device *d, const char *sysfs) {
        ...
        r = hashmap_ensure_allocated(&UNIT(d)->manager->devices_by_sysfs, string_hash_func, string_compare_func);
        if (r < 0)
                return r;
...
static int device_update_description(Unit *u, struct udev_device *dev, const char *path) {
        ...
        if (r < 0)
                log_error_unit(u->id, "Failed to set device description: %s", strerror(-r));
...
static int device_add_udev_wants(Unit *u, struct udev_device *dev) {
        ...
        FOREACH_WORD_QUOTED(word, l, wants, state) {
                ...
                if (r < 0) {
                        log_error_unit(u->id, "Failed to add wants dependency: %s", strerror(-r));
			return r;
		}
        }
        if (!isempty(state))
                log_warning_unit(u->id, "Property %s on %s has trailing garbage, ignoring.", property, strna(udev_device_get_syspath(dev)));
...
fail:
        log_warning_unit(u->id, "Failed to set up device unit: %m");
...
int device_found_node(Manager *m, const char *node, bool add, DeviceFound found, bool now) {
        ...
        if (add) {
                ...
                if (stat(node, &st) < 0) {
                        ...
                        log_error("Failed to stat device node file %s: %m", node);
                        return -errno;
                }
